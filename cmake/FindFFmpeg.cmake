...
macro(FFmpeg_find_component component)
  ...
  find_path(
    FFmpeg_${component}_INCLUDE_DIR
    NAMES ${component_libname}/${component_header}
          ${component_libname}/version.h
  ...
  if(FFmpeg_${component}_LIBRARY AND FFmpeg_${component}_INCLUDE_DIR)
    ...
    mark_as_advanced(FFmpeg_${component}_LIBRARY
                     FFmpeg_${component}_INCLUDE_DIR FFmpeg_${component}_IMPLIB)
  endif()
endmacro()
...
macro(FFmpeg_check_version)
  ...
  elseif(EXISTS
    ...
    if(EXISTS
      ...
      string(REGEX REPLACE ".*VERSION_MAJOR[ \t]+([0-9]+).*" "\\1"
                           _version_major "${_version_string}")

      file(STRINGS
           "${FFmpeg_${component}_INCLUDE_DIR}/${component_libname}/version.h"
           _version_string REGEX "^.*VERSION_(MINOR|MICRO)[ \t]+[0-9]+[ \t]*$")
      string(REGEX REPLACE ".*VERSION_MINOR[ \t]+([0-9]+).*" "\\1"
                           _version_minor "${_version_string}")
      string(REGEX REPLACE ".*VERSION_MICRO[ \t]+([0-9]+).*" "\\1"
...
macro(FFmpeg_set_soname)
  if(CMAKE_SYSTEM_NAME MATCHES "Darwin")
    ...
    if(_result EQUAL 0 AND _output MATCHES "^@rpath/")
      set_property(TARGET FFmpeg::${component} PROPERTY IMPORTED_SONAME
                                                        "${_output}")
    endif()
  elseif(CMAKE_SYSTEM_NAME MATCHES "Linux|FreeBSD")
    ...
    if(_result EQUAL 0)
      string(REGEX REPLACE "[ \t]+SONAME[ \t]+([^ \t]+)" "\\1" _soname
                           "${_output}")
      set_property(TARGET FFmpeg::${component} PROPERTY IMPORTED_SONAME
                                                        "${_soname}")
      unset(_soname)
...
if(EXISTS "${FFmpeg_avutil_INCLUDE_DIR}/libavutil/ffversion.h")
  file(STRINGS "${FFmpeg_avutil_INCLUDE_DIR}/libavutil/ffversion.h"
       _version_string
       REGEX "^.*FFMPEG_VERSION[ \t]+\"n?[0-9a-z\\~.-]+\"[ \t]*$")
  string(REGEX REPLACE ".*FFMPEG_VERSION[ \t]+\"n?([0-9]+\\.[0-9]).*\".*" "\\1"
                       FFmpeg_VERSION "${_version_string}")
endif()
...
if(CMAKE_SYSTEM_NAME MATCHES "Darwin|Windows")
  set(FFmpeg_ERROR_REASON
      "Ensure that deps is provided as part of CMAKE_PREFIX_PATH.")
elseif(CMAKE_SYSTEM_NAME MATCHES "Linux|FreeBSD")
  set(FFmpeg_ERROR_REASON
      "Ensure that required FFmpeg libraries are installed on the system.")
endif()
...
foreach(component IN LISTS FFmpeg_FIND_COMPONENTS)
  if(FFmpeg_${component}_FOUND AND NOT TARGET FFmpeg::${component})
    if(IS_ABSOLUTE "${FFmpeg_${component}_LIBRARY}")
      if(DEFINED FFmpeg_${component}_IMPLIB)
        if(FFmpeg_${component}_IMPLIB STREQUAL FFmpeg_${component}_LIBRARY)
          ...
          set_property(TARGET FFmpeg::${component}
                       PROPERTY IMPORTED_IMPLIB "${FFmpeg_${component}_IMPLIB}")
        endif()
      else()
      ...
      set_property(TARGET FFmpeg::${component}
                   PROPERTY IMPORTED_LOCATION "${FFmpeg_${component}_LIBRARY}")
    else()
      add_library(FFmpeg::${component} INTERFACE IMPORTED)
      set_property(TARGET FFmpeg::${component}
                   PROPERTY IMPORTED_LIBNAME "${FFmpeg_${component}_LIBRARY}")
    endif()
    ...
    get_target_property(_ffmpeg_interface_libraries FFmpeg::FFmpeg
