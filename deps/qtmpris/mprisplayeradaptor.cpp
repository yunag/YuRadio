...
static const QString mprisPlayerInterface =
...
MprisPlayerAdaptor::MprisPlayerAdaptor(MprisPlayer *parent)
  ...
  connect(parent, SIGNAL(canControlChanged()), this,
          SLOT(onCanControlChanged()));
  connect(parent, SIGNAL(canGoNextChanged()), this, SLOT(onCanGoNextChanged()));
  connect(parent, SIGNAL(canGoPreviousChanged()), this,
          SLOT(onCanGoPreviousChanged()));
  ...
  connect(parent, SIGNAL(loopStatusChanged()), this,
          SLOT(onLoopStatusChanged()));
  connect(parent, SIGNAL(maximumRateChanged()), this,
          SLOT(onMaximumRateChanged()));
  connect(parent, SIGNAL(metadataChanged()), this, SLOT(onMetadataChanged()));
  connect(parent, SIGNAL(minimumRateChanged()), this,
          SLOT(onMinimumRateChanged()));
  connect(parent, SIGNAL(playbackStatusChanged()), this,
          SLOT(onPlaybackStatusChanged()));
...
QString MprisPlayerAdaptor::loopStatus() const {
  return Mpris::enumerationToString(
    static_cast<MprisPlayer *>(parent())->loopStatus());
}

void MprisPlayerAdaptor::setLoopStatus(const QString &value) {
  MprisPlayer *const player = static_cast<MprisPlayer *>(parent());
  if (player->canControl()) {
    Q_EMIT player->loopStatusRequested(
      Mpris::enumerationFromString<Mpris::LoopStatus>(value));
    return;
  }
  ...
  // complain here
  qDebug() << Q_FUNC_INFO
           << "Requested to modify the loop status but it is not supported.";
}
...
QString MprisPlayerAdaptor::playbackStatus() const {
  return Mpris::enumerationToString(
    static_cast<MprisPlayer *>(parent())->playbackStatus());
}
...
void MprisPlayerAdaptor::setRate(double value) {
  ...
  if (!player->canControl()) {
    errorMessage =
      QStringLiteral("Requested to modify the rate but it is not supported.");
  } else if (player->minimumRate() > value) {
    errorMessage = QStringLiteral(
      "Requested to modify the rate under its minimum allowed value.");
  } else if (player->maximumRate() < value) {
    errorMessage = QStringLiteral(
      "Requested to modify the rate over its maximum allowed value.");
  }
...
void MprisPlayerAdaptor::setShuffle(bool value) {
  ...
  // complain here
  qDebug() << Q_FUNC_INFO
           << "Requested to modify the shuffle but it is not supported.";
}
...
void MprisPlayerAdaptor::setVolume(double value) {
  ...
  // complain here
  qDebug() << Q_FUNC_INFO
           << "Requested to modify the volume but it is not supported.";
}

void MprisPlayerAdaptor::Next() {
  MprisPlayer *const player = static_cast<MprisPlayer *>(parent());
  if (!player->canControl()) {
    player->sendErrorReply(
      QDBusError::NotSupported,
      QStringLiteral("Wanted to move to next track but it is not supported."));
    return;
  }
...
void MprisPlayerAdaptor::OpenUri(const QString &Uri) {
  ...
  if (!player->canControl()) {
    player->sendErrorReply(
      QDBusError::NotSupported,
      QStringLiteral("Wanted to open an url but it is not supported."));
    return;
  }
  ...
  if (!url.isValid()) {
    player->sendErrorReply(
      ...
      QStringLiteral("Wanted to open an url but the url is invalid."));
  }

  if (!player->supportedUriSchemes().contains(url.scheme())) {
    player->sendErrorReply(
      QDBusError::NotSupported,
      QStringLiteral("Wanted to open an url but the scheme is not supported."));
  }
  ...
  player->sendErrorReply(
    QDBusError::NotSupported,
    QStringLiteral(
      "Wanted to open an url but the mime type is not supported."));
}
...
void MprisPlayerAdaptor::Pause() {
  ...
  if (!player->canControl()) {
    player->sendErrorReply(
      QDBusError::NotSupported,
      QStringLiteral("Wanted to pause but it is not supported."));
    return;
  }
...
void MprisPlayerAdaptor::Play() {
  ...
  if (!player->canControl()) {
    player->sendErrorReply(
      QDBusError::NotSupported,
      QStringLiteral("Wanted to play but it is not supported."));
    return;
  }
...
void MprisPlayerAdaptor::PlayPause() {
  ...
  if (!player->canControl()) {
    player->sendErrorReply(
      QDBusError::NotSupported,
      QStringLiteral("Wanted to play or pause but it is not supported."));
    return;
  }
...
void MprisPlayerAdaptor::Previous() {
  ...
  if (!player->canControl()) {
    player->sendErrorReply(
      QDBusError::NotSupported,
      QStringLiteral(
        "Wanted to move to previous track but it is not supported."));
    return;
  }
...
void MprisPlayerAdaptor::Seek(qlonglong Offset) {
  ...
  if (!player->canControl()) {
    player->sendErrorReply(
      QDBusError::NotSupported,
      QStringLiteral("Wanted to seek but it is not supported."));
    return;
  }
...
void MprisPlayerAdaptor::SetPosition(const QDBusObjectPath &TrackId,
  ...
  if (!player->canControl()) {
    player->sendErrorReply(
      QDBusError::NotSupported,
      QStringLiteral("Wanted to move to position but it is not supported."));
    return;
  }
  ...
  if (trackId.value<QDBusObjectPath>() != TrackId) {
    player->sendErrorReply(
      QDBusError::InvalidArgs,
      QStringLiteral(
        "Wanted to move to position but the TrackId is not the current one."));
    return;
  }
  ...
  if (Position > length.toLongLong()) {
    player->sendErrorReply(
      QDBusError::InvalidArgs,
      QStringLiteral(
        "Wanted to move to position but the position is off range."));
    return;
  }
...
void MprisPlayerAdaptor::Stop() {
  ...
  if (!player->canControl()) {
    player->sendErrorReply(
      QDBusError::NotSupported,
      QStringLiteral("Wanted to stop but it is not supported."));
    return;
  }
...
void MprisPlayerAdaptor::onCanControlChanged() const {
  ...
  if (player->canGoNext()) {
    changedProperties[QStringLiteral("CanGoNext")] =
      QVariant(player->canControl());
  }
  if (player->canGoPrevious()) {
    changedProperties[QStringLiteral("CanGoPrevious")] =
      QVariant(player->canControl());
  }
  if (player->canPause()) {
    changedProperties[QStringLiteral("CanPause")] =
      QVariant(player->canControl());
  }
  if (player->canPlay()) {
    changedProperties[QStringLiteral("CanPlay")] =
      QVariant(player->canControl());
  }
  if (player->canSeek()) {
    changedProperties[QStringLiteral("CanSeek")] =
      QVariant(player->canControl());
  }
  ...
  player->notifyPropertiesChanged(mprisPlayerInterface, changedProperties,
                                  QStringList());
}
...
void MprisPlayerAdaptor::onLoopStatusChanged() const {
  ...
  QVariantMap changedProperties;
  changedProperties[QStringLiteral("LoopStatus")] =
    QVariant(Mpris::enumerationToString(player->loopStatus()));
...
void MprisPlayerAdaptor::onMaximumRateChanged() const {
  ...
  QVariantMap changedProperties;
  changedProperties[QStringLiteral("MaximumRate")] =
    QVariant(player->maximumRate());
...
void MprisPlayerAdaptor::onMinimumRateChanged() const {
  ...
  QVariantMap changedProperties;
  changedProperties[QStringLiteral("MinimumRate")] =
    QVariant(player->minimumRate() < 0 ? 0 : player->minimumRate());
...
void MprisPlayerAdaptor::onPlaybackStatusChanged() const {
  ...
  QVariantMap changedProperties;
  changedProperties[QStringLiteral("PlaybackStatus")] =
    QVariant(Mpris::enumerationToString(player->playbackStatus()));
...
void MprisPlayerAdaptor::onRateChanged() const {
  ...
  if (player->rate() <= 0 || player->rate() < player->minimumRate() ||
      player->rate() > player->maximumRate()) {
    qWarning() << "Mpris: Rate should never be negative or out of the minimum "
                  "and maximum limits";
    return;
  }
...
void MprisPlayerAdaptor::onVolumeChanged() const {
  ...
  QVariantMap changedProperties;
  changedProperties[QStringLiteral("Volume")] =
    QVariant(player->volume() < 0 ? 0 : player->volume());
