...
static const QString serviceNamePrefix =
  QStringLiteral("org.mpris.MediaPlayer2.");
static const QString mprisObjectPath =
  QStringLiteral("/org/mpris/MediaPlayer2");
static const QString dBusPropertiesInterface =
  QStringLiteral("org.freedesktop.DBus.Properties");
static const QString dBusPropertiesChangedSignal =
...
MprisPlayer::MprisPlayer(QObject *parent)
    : QObject(parent), QDBusContext(),
      ...
      m_mprisPlayerAdaptor(new MprisPlayerAdaptor(this)), m_canQuit(false),
      m_canRaise(false), m_canSetFullscreen(false), m_fullscreen(false),
      m_hasTrackList(false), m_canControl(false), m_canGoNext(false),
      m_canGoPrevious(false), m_canPause(false), m_canPlay(false),
      m_canSeek(false), m_loopStatus(Mpris::None), m_maximumRate(1),
      m_minimumRate(1), m_playbackStatus(Mpris::Stopped), m_position(0),
      m_rate(1), m_shuffle(false), m_volume(0) {
  ...
  if (!connection.isConnected()) {
    ...
    qWarning() << "Mpris: Failed attempting to register object path. Already "
                  "registered?";
  }
}
...
void MprisPlayer::setSupportedUriSchemes(
...
QVariantMap MprisPlayer::typeMetadata(const QVariantMap &aMetadata) {
  ...
  while (i != aMetadata.constEnd()) {
    switch (Mpris::enumerationFromString<Mpris::Metadata>(i.key())) {
      case Mpris::TrackId:
        metadata.insert(
          i.key(), QVariant::fromValue(QDBusObjectPath(i.value().toString())));
        break;
      case Mpris::Length:
      ...
      case Mpris::Url:
        metadata.insert(i.key(),
                        QVariant::fromValue(i.value().toUrl().toString()));
        break;
      case Mpris::Album:
      case Mpris::AsText:
      case Mpris::Title:
      ...
      case Mpris::LastUsed:
        metadata.insert(i.key(), QVariant::fromValue(
                                   i.value().toDate().toString(Qt::ISODate)));
        break;
...
void MprisPlayer::registerService() {
  ...
  if (!connection.registerService(
        QString(serviceNamePrefix).append(m_serviceName))) {
    qWarning() << "Mpris: Failed attempting to register service: "
               << m_serviceName << " Already taken?";
  }
...
void MprisPlayer::unregisterService() {
  if (!m_serviceName.isEmpty()) {
    QDBusConnection connection = getDBusConnection();
    connection.unregisterService(
      QString(serviceNamePrefix).append(m_serviceName));
  }
}

void MprisPlayer::notifyPropertiesChanged(
  ...
  arguments << QVariant(interfaceName) << QVariant(changedProperties)
            << QVariant(invalidatedProperties);
