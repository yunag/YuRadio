...
public:
  enum PlaybackStatus { InvalidPlaybackStatus = -1, Playing, Paused, Stopped };

  enum LoopStatus { InvalidLoopStatus = -1, None, Track, Playlist };
...
private:
  ...
  template <typename T>
  static void getEnumStringsAndSize(const char ***strings, int *size);
  template <typename T>
  static QString enumerationToString(T enumeration);
  template <typename T>
  static T enumerationFromString(const QString &string);
