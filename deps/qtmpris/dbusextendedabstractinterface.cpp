...
DBusExtendedAbstractInterface::DBusExtendedAbstractInterface(
  ...
  const QDBusConnection &connection, QObject *parent)
    : QDBusAbstractInterface(service, path, interface, connection, parent),
      m_sync(false), m_useCache(false), m_getAllPendingCallWatcher(0),
...
void DBusExtendedAbstractInterface::disconnectNotify(
...
QVariant DBusExtendedAbstractInterface::internalPropGet(const char *propname,
                                                        void *propertyPtr) {
  m_lastExtendedError = QDBusError();
  ...
  if (m_sync) {
    ...
    if (metaProperty.userType() != QMetaType::QVariant) {
      expectedSignature =
        QDBusMetaType::typeToSignature(metaProperty.metaType());
...
void DBusExtendedAbstractInterface::internalPropSet(const char *propname,
...
QVariant
DBusExtendedAbstractInterface::asyncProperty(const QString &propertyName) {
  QDBusMessage msg = QDBusMessage::createMethodCall(
  ...
  DBusExtendedPendingCallWatcher *watcher =
    new DBusExtendedPendingCallWatcher(async, propertyName, QVariant(), this);
...
void DBusExtendedAbstractInterface::asyncSetProperty(
  ...
  QDBusPendingReply<QVariant> async = connection().asyncCall(msg);
  DBusExtendedPendingCallWatcher *watcher =
    new DBusExtendedPendingCallWatcher(async, propertyName, value, this);

  connect(watcher, SIGNAL(finished(QDBusPendingCallWatcher *)), this,
          SLOT(onAsyncSetPropertyFinished(QDBusPendingCallWatcher *)));
}

void DBusExtendedAbstractInterface::onAsyncPropertyFinished(
  DBusExtendedPendingCallWatcher *watcher) {
...
void DBusExtendedAbstractInterface::onAsyncSetPropertyFinished(
  DBusExtendedPendingCallWatcher *watcher) {
  QDBusPendingReply<QVariant> reply = *watcher;
...
void DBusExtendedAbstractInterface::onAsyncGetAllPropertiesFinished(
...
void DBusExtendedAbstractInterface::onPropertiesChanged(
...
QVariant DBusExtendedAbstractInterface::demarshall(
  ...
  if (value.userType() == qMetaTypeId<QDBusArgument>()) {
    ...
    if (expectedSignature == dbusArg.currentSignature().toLatin1()) {
      QDBusMetaType::demarshall(dbusArg, metaProperty.metaType(),
                                result.data());
      if (!result.isValid()) {
        errorMessage =
    ...
    const char *actualSignature =
      QDBusMetaType::typeToSignature(value.metaType());
